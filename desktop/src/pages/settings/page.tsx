...
import { Bot, Globe, Mic, ShieldCheck, Wrench, X } from 'lucide-react'
...
type SectionId = 'general' | 'models' | 'dictation' | 'privacy' | 'advanced'
...
export default function SettingsPage({ setVisible, scrollTo }: SettingsPageProps) {
	...
	const groups: SettingsGroup[] = [
		{
			label: m.customize(),
			sections: [{ id: 'dictation', label: m.globalDictation(), icon: <Mic className="h-4 w-4" /> }],
		},
		{
			label: m.transcription(),
			sections: [{ id: 'models', label: m.selectModel(), icon: <Bot className="h-4 w-4" /> }],
		},
		{
			label: m.general(),
			sections: [
	...
	const [activeSection, setActiveSection] = useState<SectionId>(
		sections.some((s) => s.id === scrollTo) ? (scrollTo as SectionId) : 'dictation',
	)
